...
def save_step_lost(step: int, city_originals: list, config: dict):
    """
    Save data loss file for a step. One column: city_original.
    Each row = one record lost at this step.
    """
    lost_dir = config.get("PATHS", {}).get("lost_dir")
    if not lost_dir:
        return
    os.makedirs(lost_dir, exist_ok=True)
    path = os.path.join(lost_dir, f"step{step:02d}_lost.csv")
    df = pd.DataFrame({"city_original": city_originals})
    df.to_csv(path, index=False)
    print(f"  [OK] Data loss saved -> {path}  ({len(df):,} rows lost)")
