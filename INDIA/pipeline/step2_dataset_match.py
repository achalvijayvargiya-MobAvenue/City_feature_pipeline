...
def run(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    # ── Pincode flow: data already from Pincode_mapping, skip matching ──────
    if "match_source" in df.columns and (df["match_source"] == "pincode_mapping").all():
        print("\n  Input from pincode mapping — skipping dataset match (pass-through)")
        coverage_report(df, ["state_original", "major_city", "latitude", "longitude", "match_source"])
        save_checkpoint(df, config["PATHS"]["checkpoints"], step=2)
        return df
    ...
    # ── Save data loss (city_original only) ───────────────────────────────
    lost_cities = (
        unmatched_df["city_original"].astype(str).tolist()
        if total_unmatched > 0 and "city_original" in unmatched_df.columns
        else []
    )
    save_step_lost(2, lost_cities, config)
