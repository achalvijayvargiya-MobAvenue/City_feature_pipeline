...
def run(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    # Pincode flow: skip validation (postal data trusted)
    if "match_source" in df.columns and (df["match_source"] == "pincode_mapping").all():
        print("\n  Input from pincode mapping — skipping GeoJSON validation (pass-through)")
        df = df.copy()
        df["is_valid"] = None
    ...
    before = len(df)
    invalid_mask = df["is_valid"] == False
    lost_cities = df.loc[invalid_mask, "city_original"].astype(str).tolist()
    ...
    save_step_lost(3, lost_cities, config)
