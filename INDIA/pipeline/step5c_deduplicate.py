...
"""
import pandas as pd
from utils import normalize_state, print_step_header, coverage_report, print_value_counts, save_checkpoint, save_step_lost
...
def run(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    # city_original of removed rows (duplicates)
    removed_idx = df.index.difference(best_idx)
    lost_cities = df.loc[removed_idx, "city_original"].astype(str).tolist()

    print(f"\n  Output rows           : {rows_after:,}")
    print(f"  Rows removed          : {removed:,}")
    print(f"  One-to-one mapping    : {out_df['city_original'].nunique():,} city_original → 1 major_city each")

    save_step_lost(7, lost_cities, config)
