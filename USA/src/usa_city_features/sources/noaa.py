...
class NOAAAdapter(SourceAdapter):
    ...
    def __init__(self, cache: LocalCache):
        self.cache = cache
        # Using HIFLD Coastal Counties as a proxy for coastal cities
        self.base_url = "https://services.arcgis.com/P3ePLMYs2RVChkJx/arcgis/rest/services/Coastal_Counties/FeatureServer/0/query"
    ...
    def validate(self, response: SourceResponse) -> None:
        if not response.data or "features" not in response.data:
            raise ValueError("Invalid NOAA API response format")
